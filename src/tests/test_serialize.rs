...
use ton_api::{ton::ton_node::rempmessagestatus, IntoBoxed};

include!("./test_common.rs");

fn assert_json_eq_file(json: &str, name: &str) {
    let expected = std::fs::read_to_string(format!("src/tests/data/{}-ethalon.json", name))
        .unwrap();
...
pub fn generate_big_msg() -> CommonMessage {
    ...
    let code3 = SliceData::new(
        vec![
            0xad, 0xc9, 0xba, 0xfc, 0x56, 0x94, 0x11, 
            0x57, 0x58, 0xfa, 0x2b, 0xdf, 0xe4, 0x65, 
            0x15, 0x1a, 0x32, 0x03, 0x69, 0x4a, 0xff, 
            0xcd, 0x00, 0x8f, 0x36, 0x8b, 0xd2, 0xcc, 
            0x8c, 0xc8, 0x10, 0xfb, 0x6b, 0x5b, 0x51
        ]
...
fn test_account_into_json_without_hash_0() {
    ...
    assert_eq!(format!("{:#}", serde_json::json!(json)),
r#"{
  "json_version": 8,
...
fn test_pruned_account_into_json_0() {
    ...
    let proof = MerkleProof::create(
        &cell,
        |hash| Some(hash) != code.as_ref() && Some(hash) != libs.as_ref()
    ).unwrap();
...
#[test]
fn test_get_config() {
  let filename = "src/tests/data/9C9906A80D020952E0192DC60C0B2BF1F55FE9A9E065606E8FE25C08BD1AA6B2.boc";
...
fn test_crafted_key_block_into_json() {
    ...
    let cp3 = ConfigParamEnum::ConfigParam3(ConfigParam3 { fee_collector_addr: UInt256::from([133;32]) });
    let cp4 = ConfigParamEnum::ConfigParam4(ConfigParam4 { dns_root_addr: UInt256::from([144;32]) });
    let cp6 = ConfigParamEnum::ConfigParam6(
        ConfigParam6 {
...
fn test_balance_delta() {
  ...
  check_transaction_field(
    "ext_in&int_out_special.boc", "balance_delta", "-f2d92301e7d945ff", "-0x26dcfe1826ba00"
  );
...
fn se_deserialise_remp_status(status: RempMessageStatus) {
    let rr = ton_api::ton::ton_node::rempreceipt::RempReceipt {
        message_id: "18AFCDD25BE0989CE516504263EB351818A0FF8F6AB3689501C8E3B767EF413C".parse().unwrap(),
        status,
        timestamp: 1640011209924,
        source_id: "18AFCDD25BE0989CE516504263EB351818A0FF8F6AB368950888E3B767EF413C".parse().unwrap(),
    }.into_boxed();
